...
function createMobileMenu() {
    ...
    $('.mobile-nav-item').each(function() {
        var $original = $(this);
        var mobileNavItem = $original.clone().addClass('mobile-nav-link');

        mobileNavItem.removeAttr('role');
        mobileNavItem.find('a').attr('role', 'menuitem');

        // Check for and fix language form ID conflicts
        var langForm = mobileNavItem.find('#language-settings-form');
        if (langForm.length > 0) {
            var mobileLangForm = langForm.clone();
            mobileLangForm.attr('id', 'language-settings-form-mobile');
            mobileLangForm.find('#settings-language-value').attr('id', 'settings-language-value-mobile');
            mobileLangForm.find('.url-endpoint').attr('title', 'mobile-endpoint');

            langForm.replaceWith(mobileLangForm);
        }
    ...
    // Re-bind language selector logic after menu is built
